...
power_persym = 10.^(SNR_persym/10); % 每个符号的能量%每个符号的能量
...
for i_snr=1:length(SNR_persym)
    ...
    while  (nums*bit )<iter(i_snr)
        ...
        if denominator > 0
            power_per_central_symbol = power / denominator;
        else
        ...
        for l=0:M-1
            for k=0:N-1
                for i=1:P
                    ...
                    hw(l+1,k+1)=hw(l+1,k+1)+h_p_round(i)*h_exp(i)*delta_term*zeta_N(k-ki(i),N).*theta;
                end
            end
        end
        mag_hw = abs(hw);
        ...
        if mod(nums, 50) == 0 % 每50次迭代打印一次状态
            fprintf('SNR: %.1f dB | Iter: %d | progress: %.2f%% | Current BER: %e\n', ...
                SNR_persym(i_snr), nums, roundn(total_bits_transmitted/iter(i_snr),-2)*100, total_bit_errors/total_bits_transmitted);
        end
