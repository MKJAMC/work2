...
for i_snr=1:length(SNR)
    ...
    parfor f=1:num_frames
        ...
        Y=y+noise;
        YABS=abs(Y);[max_values0, max_indices0] = max(YABS, [], 2);%选出了每一行的最大值
      ...
      %% 互相关信道估计
        ...
        %% MMSE 检测
        H_freq_est=fft2(hw_est);
        y_freq = fft2(Y_data_only); % y已经是矩阵形式，无需reshape
        alpha = 1 / factor;
        H_mmse_freq = conj(H_freq_est) ./ (abs(H_freq_est).^2 + alpha);
        z_freq_initial = H_mmse_freq .* y_freq;
        dd_est_matrix = ifft2(z_freq_initial);
        a1=abs(dd_est_matrix);

        % --- 步骤 5: 提取数据符号并解调 ---
        % 找到原始数据符号的位置 (排除保护带的0和导频)
