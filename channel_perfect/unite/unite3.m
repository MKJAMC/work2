...
for i_snr=1:length(SNR)
    ...
    for f=1:num_frames % <-- parfor loop starts here
        ...
        % This 'k' is the loop variable that caused the conflict
        for l=0:M-1
            for k=0:N-1
                for i=1:P
        ...
        for row = data_rows
            for j = 1:num_groups_data
                ...
                % 1. 找到 group_est 中幅度最大的两个位置的索引
                [~, sorted_indices] = sort(abs(group_est), 'descend');
                candidate_indices = sorted_indices(1:2);

                % 2. 只遍历这两个候选位置
                for idx = 1:length(candidate_indices)
                    % --- SOLUTION: Use a new variable name 'k_pos' instead of 'k' ---
                    k_pos = candidate_indices(idx); % k_pos 是当前的候选位置 (1到g之间)

                    % 3. 直接对该位置的信号硬判决，不遍历星座点
                    detected_symbol_int = qamdemod(group_est(k_pos), cen_modu, 'UnitAveragePower', true, 'OutputType', 'integer');
                    current_symbol = qammod(detected_symbol_int, cen_modu, 'UnitAveragePower', true);

                    % 4. 为这个唯一的(位置,符号)组合计算度量
                    metric = -abs(group_est(k_pos) - current_symbol)^2;
                    for m = 1:g
                        if m ~= k_pos
        ...
        for row = guard_rows
            ...
            while i_col <= num_guard_cols
                ...
                if current_group_size  > 1
                    ...
                    % 1. 找到候选位置
                    num_candidates = min(2, current_group_size);
                    [~, sorted_indices] = sort(abs(group_est), 'descend');
                    ...
                    for idx = 1:length(candidate_indices)
                        % --- SOLUTION: Use a new variable name 'k_pos' instead of 'k' ---
                        k_pos = candidate_indices(idx); % k_pos 是当前的候选位置

                        % 3. 直接硬判决
                        detected_symbol_int = qamdemod(group_est(k_pos), guard_modu, 'UnitAveragePower', true, 'OutputType', 'integer');
