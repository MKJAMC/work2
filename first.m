...
for i_snr=1:length(SNR_dB)
    SNR = 10^(SNR_dB(i_snr)/10);% 总功率，由SNR定义
   ...
   % while  (nums*bit )<iter(i_snr)
        ...
        for l=0:M-1
            for k=0:N-1
                for i=1:P
                    ...
                    % hw(l+1,k+1)=hw(l+1,k+1)+h_p(i)*1*delta_term*zeta_N(k-ki(i),N).*theta;
                end
            end
        end
        mag_hw = abs(hw);
        ...
        for i=1:length(max_values)%假设信道时延不为0
           if(max(YABS(i,:))>3)%判断时延是否存在的证据
                li_est=[li_est,i-1];
        ...
        disp(['不同路径的信道相位=',num2str(h_exp_round)]);
        ...
        for l=0:M-1
            for k=0:N-1
                for i=1:P
                    ...
                    % hw_est(l+1,k+1)=hw_est(l+1,k+1)+h_est(i)*1*delta_term*zeta_N(k-ki_est(i),N).*theta;
                end
            end
        end
        ...
        dd_est_vec=dd_est_vec./sqrt((SNR/ans1));
