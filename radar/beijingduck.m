% 生成：最大/最小增益比在 4.5~5.0，且 T1、T2、T3 三个目标均互相靠近（间隔均 < 1）
clc; clear; close all;
%% ================= 参数设置 =================
% 基础参数
M = 64;
...
fprintf('======================================================\n');
fprintf('开始 Monte Carlo 仿真 (三个目标均不可分辨 + 增益比在 4.5~5.0)\n');
fprintf('SNR 范围: [%s] dB\n', num2str(SNR_vec));
fprintf('======================================================\n');
...
for s_idx = 1:length(SNR_vec)
    ...
    for mc = 1:num_monte_carlo
        ...
        % ================= 【核心修改区】 =================
        target_amp_ratio_max = 5.0;  % 目标幅值比上限 (保证在5倍以内)

        % === 定义间隔约束 ===
        % 所有的目标的距离、速度间隔均 < 1 倍分辨率 (取 0.3~0.6 之间)
        gap_R_min = 0.3 * distance_res;
        gap_R_max = 0.6 * distance_res;
        gap_V_min = 0.3 * v_res;
        gap_V_max = 0.6 * v_res;
        ...
        while ~is_valid
            ...
            R2 = R1 + gap_R_min + rand * (gap_R_max - gap_R_min);
            R3 = R2 + gap_R_min + rand * (gap_R_max - gap_R_min);
            ...
            if R3 < R_max * 0.95
                ...
                actual_gap_V12 = gap_V_min + rand * (gap_V_max - gap_V_min);
                if rand > 0.5; V2 = V1 + actual_gap_V12; else; V2 = V1 - actual_gap_V12; end

                actual_gap_V23 = gap_V_min + rand * (gap_V_max - gap_V_min);
                if rand > 0.5; V3 = V2 + actual_gap_V23; else; V3 = V2 - actual_gap_V23; end

                T_Velocity = [V1, V2, V3];

                % --- C. 计算纯物理幅值 (不含任何人为空为缩放) ---
