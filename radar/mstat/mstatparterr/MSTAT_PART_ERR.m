...
for s_idx = 1:length(SNR_vec)
    ...
    for mc = 1:num_monte_carlo
        ...
        % --- 1. 生成随机目标 ---
        rng(mc);
        min_amp_ratio = 50;           % 最小幅值比 (保持不变)
        min_abs_amp   = 0.0065;       % 【修改点3】 最小绝对幅值门限 (保持你的设定)

        % 物理间隔设置
        % 【修改点1】 距离最小间隔为 1 (之前是 1.05)
        gap_R = 0* distance_res;

        % 【修改点2】 多普勒最小间隔为 0 (之前是 1.05 * v_res)
        % 设置为 0 意味着不做间隔强制约束，允许目标速度非常接近甚至相同
        gap_V = 0 * v_res;
        ...
        while ~is_valid
            ...
            % === A. 生成距离 (Range) ===
            % 策略：为了保证物理上能产生 >20 倍的幅值差，
            % 最近的目标 R1 必须离雷达比较近。

            % 1. 生成 R1 (最近目标)
            % 为了让 R3 能放下，R1 不能太远。
            % 同时也防止 R1 过于接近 0 导致幅值无穷大，设个下限 0.5 个分辨单元
            R1_min = 0.5 * distance_res;
            % R1 的上限粗略设为 R_max / sqrt(20)，否则 R3 就会跑出 R_max
            R1_max = (R_max * 0.9) / sqrt(min_amp_ratio);

            R1 = R1_min + rand * (R1_max - R1_min);

            % 2. 生成 R3 (最远目标)
            % 必须满足两个条件：
            % a. 距离 R1 足够远，使得 (R3/R1)^2 > 20
            % b. 在 R_max 范围内
            R3_min = R1 * sqrt(min_amp_ratio);
            R3_max = R_max * 0.95; % 留5%余量

            if R3_max > R3_min
                R3 = R3_min + rand * (R3_max - R3_min);

                % 3. 生成 R2 (中间目标)
                % 必须在 R1 和 R3 之间，且满足间隔 gap_R
                R2_min = R1 + gap_R;
                R2_max = R3 - gap_R;
                ...
                if R2_max > R2_min
                    ...
                    % 组合并排序
                    T_Range = sort([R1, R2, R3]);

                    % === B. 生成速度 (Velocity) ===
                    valid_vel = false;
                    vel_retry = 0;
                    while ~valid_vel && vel_retry < 100
                        % 随机生成速度
                        T_Velocity = (rand(1, num_targets) * 2 - 1) * V_max * 0.9;
                        % 检查速度间隔
                        if all(diff(sort(T_Velocity)) > gap_V)
                            valid_vel = true;
                        end
                    ...
                    if valid_vel
                        ...
                        % 1. 归一化平均功率为 1
                        % 计算当前的平均功率 P = mean(|h|^2)
                        current_power = mean(raw_trend.^2);
                        % 归一化幅度：除以 sqrt(P)，使得新信号的 mean(|h|^2) = 1
                        ...
                        if (min(true_h_abs) > 0.0065) && (current_dyn_range > min_amp_ratio)
                            is_valid = true;
            ...
            if loop_safety_count > 5000
                % 如果总是失败，说明 R_max 太小，无法容纳 >20 倍动态范围的物理衰减
                warning('无法生成满足条件的目标。请检查 R_max 是否足够大以容纳 >20 倍的衰减。');
                break;
            end
        end
