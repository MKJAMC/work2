...
for s_idx = 1:length(SNR_vec)
    ...
    for mc = 1:num_monte_carlo
        disp(mc);
        % mc=443;
        % --- 1. 生成随机目标 (修改版：T3 随机，无间隔要求，保证 T1/T3 幅度比 > 50) ---
        rng(mc);
        min_amp_ratio = 50;      % 【修改点】动态范围阈值设为 50
        min_abs_amp   = 0.0065;  % 最小绝对幅值
        
        % === 定义间隔约束 (仅针对 T1/T2) ===
        fixed_gap_R = 1 * distance_res; 
        fixed_gap_V = 1 * v_res;
        ...
        while ~is_valid
            ...
            % 1. 生成 R1 (最近目标)
            % R1 必须足够小，以便 R3 (是R1的sqrt(50)倍) 不会超出 R_max
            % sqrt(50) ≈ 7.07
            R1_min = 0.5 * distance_res; 
            R1_max = (R_max * 0.95) / sqrt(min_amp_ratio); 
            ...
            % 3. 生成 R3 (随机，仅受幅度比约束)
            % 满足 T1/T3 > 50 => (R3/R1)^2 > 50 => R3 > R1 * sqrt(50)
            ...
            if R3_max > R3_min
                ...
                % 计算 T1 与 T3 的幅度比
                ratio_T1_T3 = true_h_abs(1) / true_h_abs(3);
                
                % 最终检查
                if (min(true_h_abs) > min_abs_amp) && (ratio_T1_T3 > min_amp_ratio)
                    is_valid = true;
            ...
            if loop_safety_count > 5000
                warning('无法生成满足条件的目标 (R_max限制了50倍动态范围)，重置参数...');
                break; 
            end
        end
