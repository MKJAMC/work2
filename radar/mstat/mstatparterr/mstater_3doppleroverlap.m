%第一个和第二个目标多普勒时延间隔为2,第三个目标与前两个的多普勒时延间隔<1
clc; clear; close all;
%% ================= 参数设置 =================
...
for s_idx = 1:length(SNR_vec)
    ...
    for mc = 1:num_monte_carlo
        disp(mc);
        % mc=443;
        % --- 1. 生成随机目标 (修改版：V3 与 V1或V2 发生多普勒重叠) ---
        rng(mc);
        min_amp_ratio = 50;      % 动态范围阈值 (>50)
        min_abs_amp   = 0.0065;  % 最小绝对幅值
        
        % === 定义间隔约束 ===
        fixed_gap_R = 2 * distance_res; 
        fixed_gap_V = 2 * v_res;
        
        % V3 的多普勒偏移量约束 (小于1个分辨率单元)
        % 这里设置为 0.1 ~ 0.8 倍分辨率，确保靠得很近但不是完全重合
        overlap_factor = 0.8; 
        ...
        while ~is_valid
            ...
            % === A. 生成距离 (Range) (保持原逻辑：R1<R2<R3，且满足大幅值差) ===
            ...
            % 3. 生成 R3 (最远，满足动态范围)
            % R3 至少要在 R2 后面一点
            R3_min_gap = R2 + 1.0 * distance_res;
            % R3 必须足够远以满足衰减要求
            R3_min_dyn = R1 * sqrt(min_amp_ratio); 
            ...
            if R3_max > R3_min
                ...
                % === B. 生成速度 (Velocity) [核心修改区域] ===
                % 1. 生成 V1 (留出空间给V2)
                V1_limit = V_max - fixed_gap_V - v_res; 
                V1 = (rand * 2 - 1) * V1_limit * 0.8;
                
                % 2. 生成 V2 (与 V1 固定间隔 2 个单元)
                V2 = V1 + fixed_gap_V;
                
                % 3. 生成 V3 (强制与 V1 或 V2 重叠)
                % 随机选择靠近 V1 还是 V2
                if rand > 0.5
                    v_ref = V1;
                else
                ...
                % 生成一个小于 1 个分辨率单元的随机偏移
                % 范围在 [-0.8, 0.8] * v_res 之间
                v_offset = (rand * 2 - 1) * overlap_factor * v_res;
                V3 = v_ref + v_offset;
