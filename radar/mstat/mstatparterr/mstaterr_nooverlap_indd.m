%第一个和第二个目标多普勒时延间隔为2,第三个目标与前两个的多普勒时延最小间隔>1 
...
for s_idx = 1:length(SNR_vec)
    ...
    for mc = 1:num_monte_carlo
        disp(mc);
        % mc=443;
        % --- 1. 生成随机目标 (修改版：T1/T2 固定间隔，T3 随机远离) ---
       % --- 1. 生成随机目标 (修正版：修复动态范围逻辑错误) ---
        rng(mc);
        min_amp_ratio = 50;      % 动态范围阈值 (>50)
        min_abs_amp   = 0.0065;  % 最小绝对幅值
        
        % === 定义间隔约束 ===
        fixed_gap_R = 0.5 * distance_res; 
        fixed_gap_V = 0.5 * v_res;
        % 第三个目标
        safe_gap_R  = 1.0 * distance_res;
        safe_gap_V  = 1.0 * v_res;
        ...
        while ~is_valid
            ...
            % 1. 生成 R1 (最近目标)
            % 关键：为了让 R3 能达到 50 倍以上的衰减，R1 必须足够小。
            % R1 上限设为 R_max 除以 sqrt(50)，预留空间。
            ...
            % 3. 生成 R3 (最远目标)
            % [修正点] R3 必须足够远，才能满足动态范围 > 50
            % 下限取两个条件的最大值：(1) 物理间隔 safe_gap (2) 动态范围要求
            R3_min_gap = R2 + safe_gap_R;
            R3_min_dyn = R1 * sqrt(min_amp_ratio); % 必须大于 R1 * 7.07
            R3_min = max(R3_min_gap, R3_min_dyn);
            ...
            if R3_max > R3_min
                ...
                while ~valid_vel && vel_retry < 100
                    % 生成 V1
                    V1_limit = V_max - fixed_gap_V; 
                    V1 = (rand * 2 - 1) * V1_limit * 0.9;
                    ...
                    % 检查 V3 间隔
                    if abs(V3 - V1) > safe_gap_V && abs(V3 - V2) > safe_gap_V
                        valid_vel = true;
                        T_Velocity = [V1, V2, V3]; 
                    end
                ...
                if valid_vel
                    ...
                    % 此时应该很容易满足条件
                    if (min(true_h_abs) > min_abs_amp) && (current_dyn_range > min_amp_ratio)
                        is_valid = true;
            ...
            if loop_safety_count > 5000
                warning('无法生成满足条件的目标 (R1可能过大导致R3溢出)，重置参数...');
                break; 
            end
        end
