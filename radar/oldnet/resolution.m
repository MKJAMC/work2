...
% 初始化存储
Dataset_Y = zeros(M, N, 3, datanums); % [M, N, 3, Batch]
% 通道定义:
% Channel 1: 实部 (Real)
% Channel 2: 虚部 (Imag)
% Channel 3: 模长 (Abs/Modulus)
Target_Delay_Index = zeros(datanums, num_targets);
Target_Doppler_Index = zeros(datanums, num_targets);
delay_max= R_max * 0.9./ distance_res;  %64降到57.6 1125m
doppler_max=V_max * 0.8./v_res; % 16降到12.8  112.5m/s
...
for i_data = 1:datanums
    ...
    Dataset_Y(:, :, 3, i_data) = abs(Y);   % 通道3: 模长
